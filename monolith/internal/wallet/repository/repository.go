...
func (r *mysqlWalletRepository) CreateTx(ctx context.Context, tx *sql.Tx, w *model.Wallet) error {
	query := "INSERT INTO wallets (id, user_id, balance, currency, status) VALUES ($1, $2, $3, $4, $5)"
...
func (r *mysqlWalletRepository) GetByUserID(ctx context.Context, userID string) (*model.Wallet, error) {
	query := `SELECT id, user_id, balance, currency, status, version, created_at, updated_at FROM wallets WHERE user_id = $1 AND deleted_at IS NULL`
	w := &model.Wallet{}
