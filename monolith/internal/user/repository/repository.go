...
func (r *mysqlUserRepository) GetAll(ctx context.Context, params model.PaginationParams) ([]*model.User, int64, error) {
	...
	query := fmt.Sprintf(`SELECT id, full_name, email, role, oauth_provider, oauth_id, avatar_url, is_verified, created_at, updated_at, deleted_at FROM users WHERE deleted_at IS NULL ORDER BY %s %s LIMIT $1 OFFSET $2`, sortCol, orderDir)
