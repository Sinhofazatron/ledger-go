...
import (
	...
	"perfect_api/internal/transaction/model"
)
...
func (r *mysqlTransactionRepository) CreateTx(ctx context.Context, tx *sql.Tx, t *model.Transaction) error {
	query := `INSERT INTO transactions (id, sender_wallet_id, receiver_wallet_id, amount, description, idempotency_key, status) VALUES ($1, $2, $3, $4, $5, $6, $7)`
...
func (r *mysqlTransactionRepository) GetByIdempotencyKey(ctx context.Context, key string) (*model.Transaction, error) {
	query := `SELECT id, sender_wallet_id, receiver_wallet_id, amount, description, idempotency_key, status, created_at FROM transactions WHERE idempotency_key = $1`
	t := &model.Transaction{}
...
func (r *mysqlTransactionRepository) GetHistory(ctx context.Context, walletID string, params model.PaginationParams) ([]model.Transaction, int64, error) {
	...
	countQuery := `SELECT COUNT(*) FROM transactions WHERE (sender_wallet_id = $1 OR receiver_wallet_id = $2)`
	var total int64
	var err error
	...
	query := `SELECT id, sender_wallet_id, receiver_wallet_id,
			...
			FROM transactions WHERE (sender_wallet_id = $1 OR
			receiver_wallet_id = $2)`

	var rows *sql.Rows
	if params.Status != "" {
		query += " AND status = $3 ORDER BY " + sortColumn + " " + sortOrder + " LIMIT $4 OFFSET $5"
		rows, err = r.db.QueryContext(ctx, query, walletID, walletID, params.Status, params.Limit, params.Offset())
	} else {
		query += " ORDER BY " + sortColumn + " " + sortOrder + " LIMIT $3 OFFSET $4"
