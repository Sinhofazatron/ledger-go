...
func TestTransaction_GetByIdempotencyKey(t *testing.T) {
	...
	mock.ExpectQuery("SELECT id, sender_wallet_id, receiver_wallet_id, amount, description, idempotency_key, status, created_at FROM transactions WHERE idempotency_key = $1").
		WithArgs("idem1").
		WillReturnRows(rows)
